...
void createMatrix1()
{
    ...
    cv::Mat*  m3_8u = new cv::Mat(nrows, ncols, CV_8UC3);
    m3_8u->setTo(cv::Scalar(0,0,255));
    ...
    // Display the results from the STUDENT TASK above
     cv::namedWindow ("CV_8UC3", 1);
  cv::imshow ("CV_8UC3", *m3_8u);
     cv::waitKey(0);
}
...
int main()
{
  std::cout << " Hello, this is the create matrix program\n";
    createMatrix1();
    return 0;
}
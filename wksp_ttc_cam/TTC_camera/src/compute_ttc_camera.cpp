                      ...
                      std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC)
{
    ...
    double median;
    std::cout << "Size = " << distRatios.size() << std::endl;

    if ((distRatios.size() % 2) == 0) 
    { // Even number of data points
        std::cout << "..doing even \n";
        int higher = distRatios.size() / 2;
        int lower = higher - 1;
        median = distRatios[higher]+distRatios[lower];
        median /= 2;
    } else 
    {
        std::cout << "...doing odd\n";
        int middle = distRatios.size()/2;
        median = distRatios[middle];
    }
    double dT = 1 / frameRate;
    TTC = -dT / (1 - meanDistRatio);
    std::cout << "Mean method=" << TTC << std::endl;

    // STUDENT TASK (replacement for meanDistRatio)
    TTC = -dT/ (1 - median);
    std::cout << " Median methos=" << TTC << std::endl;
}

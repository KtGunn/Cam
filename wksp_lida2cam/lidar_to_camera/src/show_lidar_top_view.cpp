...
void showLidarTopview()
{
    ...
    cout << " Size = " << topviewImg.size() << endl;
    int ptSize = 3;
    if (true) {
	...
	for (auto it = lidarPoints.begin(); it != lidarPoints.end(); ++it) {
	    ...
	    cv::circle(topviewImg, cv::Point(x, y), ptSize, cv::Scalar(0, 0, 255), -1);
	    //std::cout << "X(" << xw  << " "<< yw << ") P(" << (uint)x << " " << (uint)y << ")\n";

	    //cv::Vec3b v3 = topviewImg.at<cv::Vec3b>((uint)y,(uint)x);
	    
	    //topviewImg.at<cv::Vec3b>(x,y)[0] = 125;
	    //topviewImg.at<cv::Vec3b>(x,y)[1] = 25;
	    //topviewImg.at<cv::Vec3b>(x,y)[2] = 215;
